...
%                 Bias =  sum(abs(DoA - obj.ThetaTrue))  / obj.k;
        ...
        function [MSE_MUSIC_1,MSE_MUSIC_2,MSE_GMUSIC_1,MSE_GMUSIC_2] = GetFirstMusicType(obj,GridInterval,GridGap,type)
            % MUSIC可以和GMUSIC同时计算
            w_theta = linspace(GridInterval(1),GridInterval(2),GridGap);
            a = @(theta) exp(1i*theta*(0:obj.N-1)')/sqrt(obj.N);
            store_output = zeros(length(w_theta),2);
            ...
            locs = locs(1:obj.k);
            DoA_MUSIC = sort(w_theta(locs),'ascend');   % sorted by same order
            MSE_MUSIC_1 = sum((DoA_MUSIC(1) - obj.ThetaTrue(1)).^2);
            MSE_MUSIC_2 = sum((DoA_MUSIC(2) - obj.ThetaTrue(2)).^2);
            ...
            locs = locs(1:obj.k);
            DoA_GMUSIC = sort(w_theta(locs),'ascend');   % sorted by same order
            MSE_GMUSIC_1 = sum((DoA_GMUSIC(1) - obj.ThetaTrue(1)).^2) / 1;
            MSE_GMUSIC_2 = sum((DoA_GMUSIC(2) - obj.ThetaTrue(2)).^2) / 1;
